---
title: "Biodiversity analysis"
subtitle: "East Gippsland (extended area)" 
format:
  html:
    embed-resources: true
    toc: true
    toc-depth: 2
editor: source
execute:
  echo: false
  message: false
  warning: false
---

```{r library}
# Library
lib <- c("targets","tarchetypes",
         "tidyverse","sf","galah","terra",
         "tidyterra","tmap","leaflet","plotly",
         "iNEXT")
invisible(lapply(lib, library, character.only = TRUE))
rm(lib)

# Targets config
tar_config_set(store = here::here("_targets"))

# Create a custom theme
theme_minimal_hpha0042 <- theme_minimal() +
  theme(
    # Plot title & subtitle
    plot.title = element_text(face = 'bold'),
    plot.title.position = "plot",
    plot.subtitle = element_text(color = "grey20", size = 12)
  )
theme_set(theme_minimal_hpha0042)
```

# Data Wragnling   

```{r}
# PREPARE DATA FOR VISUALISATION
fire1920_timeframe <- data.frame(
  start_date = as.POSIXct(c("2019-11-22", "2019-11-22")), 
  end_date = as.POSIXct(c("2020-03-01", "2020-03-01")))

# Define color palette
report_pal <- setNames(c(RColorBrewer::brewer.pal(n=8,name="Set2")[c(1,2)],
                       "#F57C00", "grey30","#FFCC80","grey70"), #"#0D47A1","#E3F2FD"
              c("parkres","fire",
                "burnt_after", "unburnt_after", "burnt_before", "unburnt_before"))
```

```{r}
# PREPARE DATA FOR STATISTICAL ESTIMATION
tar_load(sighting) # ref to _targets.R
tar_load(evc_cropped) # ref to _targets.R

# Solution 1: sighting + (evc + grid_template) 
#           = sighting + evc_gridcell = sighting_evc_gridcell ------------------
  
# 1. Create a SpatRaster grid template from scratch (extend of the evc_cropped)
grid_template <- terra::rast(
    x = evc_cropped,
    crs = "epsg:7844", # GDA2020
    resolution = 1 / 3000
  )
  
# 2. Assign unique IDs to the grid cells (cell numbers)
# The cell numbers will be used as grid cell IDs
grid_template <- terra::setValues(grid_template, 1:ncell(grid_template))

# 3. Convert sf EVC polygons to terra::SpatVector
evc_cropped_v <- terra::vect(evc_cropped[c("XGROUPNAME", "geometry")])
  
# 4. Extract evc_group for each gridcell_id  
#    by rasterizing EVC polygons XGROUPNAME to each grid cell
evc_gridcell <- terra::rasterize(
  evc_cropped_v, # SpatVector object
  grid_template, # SpatRaster grid cell template
  field = "XGROUPNAME", # EVC class
)
# Sys.time()

# Convert into df | gridcell_id | evc_group | 
evc_gridcell_df <- terra::as.data.frame(evc_gridcell, cells = TRUE) |> 
  dplyr::rename(
    gridcell_id = cell, 
    evc_group = XGROUPNAME) |> 
  dplyr::mutate(
    gridcell_id = as.character(gridcell_id),
    evc_group = as.character(evc_group))

# Checking the df
# nrow(evc_gridcell_df) == length(unique(evc_gridcell_df$gridcell_id)) # nrow = 35,275,162 unique cells (from 67553244 down to here, filtered out gridcell_id with NA evc_group)

sort(unique(evc_gridcell_df$evc_group))

# 5. Convert sf points (sighting) to coordinates
# Convert sf POINT to terra SpatVector                                          
sighting_v <- terra::vect(sighting[c("sighting_id","sighting_geometry")])

# 6. Extract gridcell_id for each sighting_id based on its coordinates
sighting_gridcell <- terra::extract(
  grid_template, 
  sighting_v) |> 
  dplyr::rename(
    "sighting_rowname" = "ID", 
    "gridcell_id" = "lyr.1") |> 
  dplyr::mutate(
    sighting_rowname = as.character(sighting_rowname),
    gridcell_id = as.character(gridcell_id))

# Get evc_group associated with gridcell_id for each sighting_id
sighting_evc_gridcell <- dplyr::left_join(
  sighting_gridcell,
  evc_gridcell_df,
  by = "gridcell_id"
)

# 7. Combine the extracted cell IDs with the original sighting data
sighting <- base::cbind(
  sighting, 
  sighting_evc_gridcell[c("gridcell_id","evc_group")]
  ) |> 
  # Exclude records without evc_group
  dplyr::filter(!is.na(evc_group))
# ------------------------------------------------------------------------------

# AMENDED Select species group & vegetation group ------------------------------
# [*] Of the 14 taxon groupings and 20 habitats, only those with ≥90 observations in burnt regions in the 18 months after fires were included for further analysis. -> 7 species groups and 12 EVC groups

# Original  
# ala_sighting |> sf::st_drop_geometry() |> select(species_group) |> unique() # 14
# sighting |> sf::st_drop_geometry() |> select(evc_group) |> unique() # 20

# Selected
# selected_species_group <- sighting |> 
#   sf::st_drop_geometry() |>
#   dplyr::filter(
#     burnt == "burnt",
#     before_after_fire == "after", time_since_fire_days <= 548) |>
#   group_by(species_group, evc_group) |>
#   summarise(n = n()) |> 
#   filter(n >= 90) |> 
#   ungroup() |> 
#   select(species_group) |> 
#   unique() |> 
#   pull()

# selected_evc_group <- sighting |> 
#   sf::st_drop_geometry() |>
#   dplyr::filter(
#     burnt == "burnt",
#     before_after_fire == "after", time_since_fire_days <= 548) |>
#   group_by(species_group, evc_group) |>
#   summarise(n = n()) |> 
#   filter(n >= 90) |> 
#   ungroup() |> 
#   select(evc_group) |> 
#   unique() |> 
#   pull()

# bring forward to _targets.R 
# cat("\"",paste(sort(selected_evc_group), collapse = "\", \""),"\"",sep = "")
selected_species_group <- c("Amphibians", "Birds", "Insects", "Mammals", "Molluscs", "Plants", "Reptiles")

selected_evc_group <- c("Coastal Scrubs Grasslands and Woodlands", "Dry Forests", "Heathlands", "Lowland Forests", "Montane Grasslands, Shrublands or Woodlands", "Rainforests", "Riparian Scrubs or Swampy Scrubs and Woodlands", "Riverine Grassy Woodlands or Forests", "Rocky Outcrop or Escarpment Scrubs", "Sub-alpine Grasslands, Shrublands or Woodlands", "Wet or Damp Forests", "Wetlands")
#-------------------------------------------------------------------------------

# 8. Subset sighting to only selected_evc_group
sighting <- sighting |> 
  dplyr::filter(evc_group %in% selected_evc_group)

# [!] CHECKING: a gridcell_id only has 1 evc_group value -> 0 = PASS
# sighting |> sf::st_drop_geometry() |> 
#   group_by(gridcell_id) |>
#   summarise(n = n_distinct(evc_group)) |> 
#   filter(n > 1) |> 
#   nrow()
```


```{r}
head(sighting, 10)
```

# Data Summary     


```{r}
# Plot megafires 2019-20 extension
# Presentation/About 
tar_load(vic_map)
tar_load(bbox_fire_1920)

# VEGETATION CLASS sf POLYGONS 
plot_evc_sf <- ggplot() + 
  geom_sf(data = vic_map) +
  geom_sf(data= evc_cropped, aes(fill= XGROUPNAME), col = "transparent") +
  coord_sf(xlim = sf::st_bbox(evc_cropped)[c("xmin", "xmax")], ylim = sf::st_bbox(evc_cropped)[c("ymin", "ymax")]) + # Zoom
  theme_minimal() +
  theme(legend.position = "none") # Hide legends, too many classes
ggsave(filename = "evc_sf.png",plot = plot_evc_sf, dpi = "print",path = here::here("image"))

# VEGETATION CLASS OF EACH GRID CELL
# Checking Plot
plot_evc_group <- ggplot() +
  tidyterra::geom_spatraster(data = evc_gridcell) +
  scale_fill_viridis_d(na.value = "transparent") +  # Optional color scale
  labs(subtitle = "Vegetation class of each grid cell",
       fill = "Vegetation class") + 
  theme_minimal() +
  theme(legend.position = "bottom")

ggsave(filename = "evc_gridcell.png",plot = plot_evc_group, dpi = "print",path = here::here("image"))
```

```{r}
sighting |> 
  sf::st_drop_geometry() |> 
  group_by(sample) |> 
  summarise(
    "s.size" = n(), # number of individuals
    "observed" = length(unique(species)) # number of species
    ) 
```




# Standardising sampling periods  

Problem: Increased sighting effort after the fires. 

```{r}
# Line plot
ggplot(
  data = sighting |> 
  sf::st_drop_geometry() |> 
  dplyr::mutate(sighting_ym = format(sighting_date, "%Y-%m")) |>
  group_by(sighting_ym) |>
  summarise(sighting_count = n())
) +
  geom_line(
    aes(x = sighting_ym, y = sighting_count, group = 1),
    color = "grey30",
    linewidth = 1
  ) +
  # Line
  geom_point(aes(x = sighting_ym, y = sighting_count),
             size = 1.5) +
  theme(aspect.ratio = 1/5) + # Plot aspect ratio y/x
  # Highlight the fire period
  geom_rect(
    data = fire1920_timeframe |> dplyr::mutate(start_date = format(start_date, "%Y-%m"), end_date = format(end_date, "%Y-%m")),
    aes(xmin = start_date, xmax = end_date, ymin = -Inf, ymax = Inf),
    fill = report_pal[["fire"]],
    alpha = 0.2
  ) +
  # Annoation
  annotate(
    "text", x = format(fire1920_timeframe$start_date[1] %m-% months(2), "%Y-%m"), y = 22000,
    label = "Wildfires 2019/20",
    fontface = "bold",
    color = report_pal[["fire"]]) +
  labs(title = "Monthly number of sightings in the East Gippsland Shire",
       subtitle = "2017 - 2021",
       x = NULL, y = NULL)
```
Solution: Create temporal sampling periods with the same duration. 
For each grid cell, extract the ALA observations for the before-after fire and burnt-unburnt treatments.  

In burnt areas:  
  
Before-After treatment   
- After: One after-fire sampling period: Fire start date time-since-fire. Maximum time-since-fire is 548 days (18 months).   
- Before: Control the time-since-fire   

<!-- [S1] -->
Step 1: Fixed post-fire period  
We established a fixed post-fire period of 548 days (18 months) for all areas impacted by the fire. This period began on the fire start date of each grid cell and extended for 18 months afterward. Reasoning behind the selection of 548 days (ref to Appendix 2). 

<!-- [S2] -->
Step 2: For pre-fire samples, extract ALA sighting observations only from grid cells with post-fire observations (cells with sightings after the fire start date). Create simulated fire_start_date instead of using the actual raw data. Hence, the time-since-fire days for these grid cells are artifical and do not exceed 548 days.  
<!-- [S2.1] -->
<!-- [S2.2] Create simulated fire_start_date instead of using the actual raw data -->

<!-- [S3] -->
Step 3: Determining the pre-fire period  
To create comparable pre-fire periods, we first defined the pre-fire period starting date by subtracting a common number of days (1029 days) from the fire start date for each grid cell.   

<!-- [S4] -->
Step 4: Creating multiple pre-fire periods  
Then, we generated nine pre-fire periods: the next pre-fire period starting were determined by offsetting the preceding one by 46 days (1.5 months). Each pre-fire period extended for 548 days (18 months) to match length of the post-fire period. The latest pre-fire period began on 22 May 2018, ensuring no overlap with the post-fire periods.  
<!-- [*] constraint -->

Purpose of Controlling the time-since-fire: This careful construction of pre-fire sampling periods ensures that the pre-fire data collection occurred over timeframes mirroring those of the post-fire period. Hence, this minimises the potential influence natural fluctuation in biodiversity on the results. This control was crucial as biodiversity naturally varies over time due to factors, such as (1) cyclical patterns of abundance of many species, and (2) the long-term trends of environmental changes or species interactions.   

<!-- [S5] -->
Step 5: Select unburnt grid cells  <!-- to-rewrite -->

<!-- [**] constraint -->
- The same number of grid cells in the burnt region assigned to each vegetation class (from which habitat was derived; see Section 2.3) was then randomly selected from areas which were not burnt by the 2019–2020 fires.  
- When burnt or unburnt regions had unequal grid cell numbers per vegetation class, grid cells were randomly removed from the treatment with the higher value, to ensure equal spatial and vegetation class coverage between treatments. <!-- Fire severity, fire date, and habitat values were then extracted for each research- grade iNaturalist observation with ≤1000 m accuracy (such that observations would only be a maximum of one grid cell offset), resulting in a dataset of species observations at 1000 m resolution, with associated fire severity, fire date, and habitat. -->

- [S5.1] Determine limiting factor for each evc_group: For each vegetation group, identify whether the burnt or unburnt cells are fewer in number. This will be the limiting factor for that group.  

- [S5.2] Set sample size then random sampling within strata: For each vegetation group, set the sample size equal to the number of cells in the limiting category (burnt or unburnt). This ensures equal representation. Then, randomly sample grid cells within each stratum: 
+ For the limiting category, select all available cells.
+ For the non-limiting category, randomly select the same number of cells as in the limiting category.  
This procedure is applied for all vegetation groups.   

- [S5.3] Randomly allocate the fire_start_date from burnt_after to unburnt_after grid cells with the same vegetation class.  

- [S5.4] Subsequent creation of unburnt “before” periods followed the same methodology as described above. 
+ [S5.4.1] Create multiple pre-fire periods for unburnt_before grid cells.
+ [S5.4.2] Select observations in unburnt_before_xxxx


```{r}
#| eval: false 
# generate pre-fire periods starting date
seq(from = as.Date("2017-07-04"), by = "46 days", length.out = 9)

format(as.Date(fire1920_timeframe$start_date[1]) - 548, "%d %b %Y")  # "2018-05-22"
as.Date("2019-11-22") - 548
as.Date("2018-05-22") + 548
as.Date("2019-11-22") - as.Date("2018-02-19")
```

#### sighting_final

```{r}
# Set seed for reproducibility (stratified sampling procedure)
set.seed(5543)

# burnt_after_gridcell: grid cells with post-fire observations 
burnt_after_gridcell <- sighting |>
  sf::st_drop_geometry() |>
  # [S1] Fixed post-fire period  
  dplyr::filter(before_after_fire == "after" & time_since_fire_days <= 548) |> 
  select(gridcell_id, evc_group, fire_start_date) |>
  unique() |> # 10,932 unique gridcell_id
  # [S4] Creating multiple pre-fire periods 
  dplyr::slice(rep(1:n(), each = 9)) |>             # Replicate each row 9 times (9 pre-fire periods)
  dplyr::group_by(gridcell_id, fire_start_date) |>  # Group by original columns
  dplyr::mutate(pfp_id = row_number()) |>           # Create ID column for each replication (pre-fire period ID)
  ungroup() |> 
  dplyr::mutate(
    pfp_start_date = fire_start_date - 1029 + (pfp_id - 1) * 46, # [S3] Determining the pre-fire period  
    pfp_end_date   = pfp_start_date + 548) |> 
  # [*] S4 constraint: The latest pre-fire period began on 22 May 2018, ensuring no overlap with the post-fire periods.
  dplyr::filter(pfp_start_date <= as.Date(fire1920_timeframe$start_date[1]) - 548) # "2018-05-22" 

# Number of grid cell per vegetation class?  
sample_size_by_evc <- 
  dplyr::left_join(
    burnt_after_gridcell |>
      dplyr::filter(pfp_id == 1) |>
      dplyr::select(c(gridcell_id, evc_group)) |>
      group_by(evc_group) |>
      summarise(burnt_after_gridcell_count = n_distinct(gridcell_id))
    ,
    sighting |> sf::st_drop_geometry() |> 
      dplyr::filter(burnt == "unburnt" & before_after_fire == "after") |> # PART 3/4: sample_id = "unburnt_after"
      group_by(evc_group) |>
      summarise(unburnt_after_gridcell_count = n_distinct(gridcell_id))
    ,
    by = c("evc_group")
) |> 
  # [S5.1] Determine limiting factor for each evc_group
  dplyr::mutate(
    sample_size = pmin(burnt_after_gridcell_count,  unburnt_after_gridcell_count),
    which_treatment = case_when(
      burnt_after_gridcell_count <= unburnt_after_gridcell_count ~ "burnt_after",
      burnt_after_gridcell_count > unburnt_after_gridcell_count ~ "unburnt_after")) 

sample_size_by_evc

# [S5.2.1] Perform stratified sampling for burnt_after
burnt_after_gridcell_stratified_sampled <- 
  # Data source
  burnt_after_gridcell |> dplyr::filter(pfp_id==1) |> dplyr::select(evc_group, gridcell_id, fire_start_date) |> 
  # Left join with sample_size_by_evc
  dplyr::left_join(sample_size_by_evc |> select(c(evc_group, sample_size)),
            by = "evc_group") |> 
  dplyr::group_by(evc_group) %>% 
  dplyr::group_modify(~ slice_sample(.x, n = .x$sample_size[1])) |> # Use `slice_sample` with `n` determined by `sample_size` in each evc_group
  dplyr::ungroup() |> 
  dplyr::select(evc_group, gridcell_id, fire_start_date) 

# [S5.2.2] Perform stratified sampling for unburnt_after
unburnt_after_gridcell_stratified_sampled <- 
  # Data source = # PART 3/4: sample_id = "unburnt_after" 
  sighting |> sf::st_drop_geometry() |> 
    dplyr::filter(burnt == "unburnt" & before_after_fire == "after") |> 
    dplyr::select(evc_group, gridcell_id) |> 
    unique() |> 
  # Left join with sample_size_by_evc
  dplyr::left_join(sample_size_by_evc |> select(c(evc_group, sample_size)),
            by = "evc_group") |> 
  dplyr::group_by(evc_group) %>% 
  dplyr::group_modify(~ slice_sample(.x, n = .x$sample_size[1])) |> # Use `slice_sample` with `n` determined by `sample_size` in each evc_group
  dplyr::ungroup() |> 
  dplyr::select(evc_group, gridcell_id) |> 
  # [S5.3] Randomly allocate the fire_start_date from burnt_after to unburnt_after grid cells with the same vegetation class
  cbind(
    burnt_after_gridcell_stratified_sampled |> 
      dplyr::group_by(evc_group) |> # Group by evc_group
      slice_sample(prop = 1) |> # Shuffle within each evc_group by sampling all rows
      ungroup() |> 
      dplyr::rename(fire_start_date_shuffled = fire_start_date) |> 
      select(fire_start_date_shuffled)
    # [!] Checking proportion of each evc_group in the shuffled sample
    # cbind(burnt_after_gridcell_stratified_sampled) |> 
    # group_by(evc_group, fire_start_date_shuffled) |>
    # summarise(n=n()) |> 
    # mutate(prop = n/sum(n))
  ) |> 
  dplyr::rename(fire_start_date = fire_start_date_shuffled)

# #[!] Checking: [**] constraint of Number of grid cells in the burnt region = in the unburnt region -> 0 = PASS
# cbind(
#   burnt_after_gridcell_stratified_sampled |>
#     group_by(evc_group) |>
#     summarise(burnt_after_sample_size = n_distinct(gridcell_id)),
#   unburnt_after_gridcell_stratified_sampled |>
#     group_by(evc_group) |>
#     summarise(unburnt_after_sample_size = n_distinct(gridcell_id)) |>
#     dplyr::select(unburnt_after_sample_size)
# ) |>
#   mutate(checking = burnt_after_sample_size - unburnt_after_sample_size)
# #---

# [S5.4]
# [S5.4.1] Create multiple pre-fire periods for unburnt_before grid cells  
unburnt_after_gridcell <- unburnt_after_gridcell_stratified_sampled |> 
  select(gridcell_id, evc_group, fire_start_date) |>
  # [S4] Creating multiple pre-fire periods 
  dplyr::slice(rep(1:n(), each = 9)) |>             # Replicate each row 9 times (9 pre-fire periods)
  dplyr::group_by(gridcell_id, fire_start_date) |>  # Group by original columns
  dplyr::mutate(pfp_id = row_number()) |>           # Create ID column for each replication (pre-fire period ID)
  ungroup() |> 
  dplyr::mutate(
    pfp_start_date = fire_start_date - 1029 + (pfp_id - 1) * 46, # [S3] Determining the pre-fire period  
    pfp_end_date   = pfp_start_date + 548) |> 
  # [*] S4 constraint: The latest pre-fire period began on 22 May 2018, ensuring no overlap with the post-fire periods.
  dplyr::filter(pfp_start_date <= as.Date(fire1920_timeframe$start_date[1]) - 548) # "2018-05-22" 

# #[!] Checking: unburnt_after_gridcell_stratified_sampled$gridcell_id is unique -> 0 = PASS
# length(unique(unburnt_after_gridcell_stratified_sampled$gridcell_id)) - nrow(unburnt_after_gridcell_stratified_sampled)
# #---

# APPENDING TOWARD THE FINAL DATASET -------------------------------------------
sighting_final <- 
rbind(
  # PART 1/4: sample_id = "burnt_after"
  sighting |> 
    sf::st_drop_geometry() |> 
    dplyr::filter(before_after_fire == "after" & time_since_fire_days <= 548,
                  gridcell_id %in% unique(burnt_after_gridcell_stratified_sampled$gridcell_id)) |> 
    dplyr::mutate(sample_id = "burnt_after") |> 
    # Column structure
    select(c("sighting_id", "species_group", "species", "sighting_date", "evc_group",
             "fire_no", "fire_start_date", "time_since_fire_days",
             "before_after_fire", "burnt", "sample", "gridcell_id", "sample_id")) 
  ,
  # PART 2/4: sample_id = "burnt_before_xxxx"
  dplyr::left_join(
    # [S2] 
    # [S2.1] Extract ALA sighting observations only from grid cells with post-fire observations
    ## x (left data)
    sighting |> sf::st_drop_geometry() |>  
      dplyr::filter(burnt == "burnt" & before_after_fire == "before"),
    ## y (right data)
    burnt_after_gridcell |> 
      dplyr::filter(gridcell_id %in% unique(burnt_after_gridcell_stratified_sampled$gridcell_id)) |> 
      dplyr::select(-c(fire_start_date, evc_group))
    ,
    join_by(gridcell_id, between(x$sighting_date, y$pfp_start_date, y$pfp_end_date, bounds = "[]"))
    ) |> 
    dplyr::filter(!is.na(pfp_id)) |> 
    # [S2.2] Create simulated fire_start_date instead of using the actual raw data ---
      dplyr::select(-c(fire_start_date)) |>              # Remove actual raw fire_start_date
      dplyr::rename(fire_start_date = pfp_start_date) |> # Simulated fire_start_date
      dplyr::mutate(                                     # Recalculate time_since_fire_days
        time_since_fire_days = as.numeric(lubridate::ymd(sighting_date) - lubridate::ymd(fire_start_date))) |> 
    # ---
  # sample_id
  dplyr::mutate(sample_id = case_match(
    pfp_id,
    1  ~ 'burnt_before_1029',
    2  ~ 'burnt_before_983',
    3  ~ 'burnt_before_937',
    4  ~ 'burnt_before_891',
    5  ~ 'burnt_before_845',
    6  ~ 'burnt_before_799',
    7  ~ 'burnt_before_753',
    8  ~ 'burnt_before_707',
    9  ~ 'burnt_before_661'
    )
  ) |> 
    # Column structure
    select(c("sighting_id", "species_group", "species", "sighting_date", "evc_group",
             "fire_no", "fire_start_date", "time_since_fire_days",
             "before_after_fire", "burnt", "sample", "gridcell_id", "sample_id"))
  ,
  # PART 3/4: sample_id = "unburnt_after"  
  sighting |> sf::st_drop_geometry() |> 
    dplyr::filter(burnt == "unburnt" & before_after_fire == "after") |>
    dplyr::select(-c(fire_start_date)) |> 
    # Subset only sampled gridcell_id
    dplyr::inner_join(unburnt_after_gridcell_stratified_sampled, by = c("gridcell_id", "evc_group")) |>
    dplyr::mutate(
      sample_id = "unburnt_after",
      time_since_fire_days = as.numeric(lubridate::ymd(sighting_date) - lubridate::ymd(fire_start_date))
      ) |> 
    dplyr::filter(time_since_fire_days <= 548) |> # [*] constraint in [S1]
    # Column structure
    select(c("sighting_id", "species_group", "species", "sighting_date", "evc_group",
             "fire_no", "fire_start_date", "time_since_fire_days",
             "before_after_fire", "burnt", "sample", "gridcell_id", "sample_id")) 
  ,
  # PART 4/4: sample_id = "unburnt_before_xxxx"
  # [S5.4.2]
	dplyr::left_join(
	  # [S2]
	  # [S2.1] Extract ALA sighting observations only from grid cells with post-fire observations
	  ## x (left data)
	  sighting |> sf::st_drop_geometry() |>  
		dplyr::filter(burnt == "unburnt" & before_after_fire == "before"),
	  ## y (right data)
	  unburnt_after_gridcell |> 
		dplyr::select(-c(fire_start_date, evc_group))
	  ,
	  join_by(gridcell_id, between(x$sighting_date, y$pfp_start_date, y$pfp_end_date, bounds = "[]"))
	  ) |> 
	  dplyr::filter(!is.na(pfp_id)) |> 
	  # [S2.2] Create simulated fire_start_date instead of using the actual raw data ---
		  dplyr::select(-c(fire_start_date)) |>              # Remove actual raw fire_start_date
		  dplyr::rename(fire_start_date = pfp_start_date) |> # Simulated fire_start_date
		  dplyr::mutate(                                     # Recalculate time_since_fire_days
			  time_since_fire_days = as.numeric(lubridate::ymd(sighting_date) - lubridate::ymd(fire_start_date))) |> 
		# ---
	  # sample_id
	  dplyr::mutate(sample_id = case_match(
		  pfp_id,
		  1  ~ 'unburnt_before_1029',
		  2  ~ 'unburnt_before_983',
		  3  ~ 'unburnt_before_937',
		  4  ~ 'unburnt_before_891',
		  5  ~ 'unburnt_before_845',
		  6  ~ 'unburnt_before_799',
		  7  ~ 'unburnt_before_753',
		  8  ~ 'unburnt_before_707',
		  9  ~ 'unburnt_before_661'
	    )
	  ) |> 
		# Column structure
		select(c("sighting_id", "species_group", "species", "sighting_date", "evc_group",
				 "fire_no", "fire_start_date", "time_since_fire_days",
				 "before_after_fire", "burnt", "sample", "gridcell_id", "sample_id")) 
) |> 
  dplyr::filter(time_since_fire_days >= 0) |> 
  # Add column: time-since-fire period (months)
  dplyr::mutate(
    tsf_period = case_match(
      time_since_fire_days,
      0:91 ~ "0-3",
      92:182 ~ "3-6",
      183:274 ~ "6-9",
      275:365 ~ "9-12",
      366:456 ~ "12-15",
      457:548 ~ "15-18"
    )
  ) 

# Checking: n_distinct_gc PER evc_group
# burnt_after_gridcell |> group_by(evc_group) |> summarise(n_distinct_gc = n_distinct(gridcell_id)) 
# sighting_final |> group_by(evc_group, burnt) |> summarise(n_distinct_gc = n_distinct(gridcell_id)) 

# [!] Checking: time_since_fire_days is within 548 d -> 0 = PASS
# max(sighting_final$time_since_fire_days) - 548
# ------------------------------------------------------------------------------

# How many grid cell per sample_id? 
# sighting_final |>
#   sf::st_drop_geometry() |>
#   group_by(sample_id) |>
#   summarise(gridcell_count = n_distinct(gridcell_id))
```


Number of burnt and unburnt grid cells per vegetation class  

```{r}
sighting_final |> 
  group_by(evc_group, burnt) |> 
  summarise(gridcell_count = n_distinct(gridcell_id)) |> 
  pivot_wider(names_from = burnt, values_from = gridcell_count) |> 
  dplyr::mutate(gridcell_diff = burnt - unburnt)
```
# Statistical Estimation   

### Standardising Sample Completeness (SC)   

**Sample Completeness** the extent to which a sample captures the true diversity of the assemblage it represents. It's a general concept that reflects how well the observed species in a sample represent the total species pool. A higher sample completeness implies that a larger proportion of the assemblage's true diversity has been sampled.

**Sample Coverage** a specific metric used to quantify sample completeness. It is expressed as a percentage or proportion, with 100% (or 1) indicating complete coverage, meaning all species in the assemblage have been detected


**Base Coverage** This is a specific coverage value selected for standardizing comparisons between multiple assemblages when using coverage-based rarefaction and extrapolation curves.

The suggested base coverage by `iNEXT::iNEXT()` is used to standardize the level of sample completeness when comparing the diversity of multiple assemblages.

**Sampling completeness**  
Sampling completeness: how well the collected sample represents the true diversity of the entire community.  

The importance of controlling the Sampling completeness: It is essential to control for sampling completeness because diversity estimates (like species richness, Shannon diversity, and Simpson diversity) tend to increase as sampling effort and completeness rise. Failure to account for variations in sampling effort can introduce biases and lead to inaccurate comparisons between different treatments or groups.

#### sp1: all species groups  

```{r}
# [sp1] all species groups
# [SE1] TRANSFORM INTO iNEXT datatype="incidence_freq" SUPPORTED INPUT FORMAT
sp1 <- rbind(
  #[1] T (NUMBER OF SAMPLING UNITS)
  sighting_final |>
    # Filter-out
    filter(!species_group %in% c("Molluscs"),
           !evc_group %in% c("Riverine Grassy Woodlands or Forests","Rocky Outcrop or Escarpment Scrubs")) |>
    group_by(sample_id) |> 
    summarise(T = n_distinct(gridcell_id)) |> 
    pivot_wider(names_from = sample_id, values_from = T, values_fill = NULL)
  ,
  #[2] SPECIES INCIDENCE FREQUENCIES: Number of grid cells in which ith species was sighted
  sighting_final |> 
    # Filter-out
    filter(!species_group %in% c("Molluscs"),
           !evc_group %in% c("Riverine Grassy Woodlands or Forests","Rocky Outcrop or Escarpment Scrubs")) |>
    group_by(sample_id, species) |> 
    summarise(y.i = n_distinct(gridcell_id)) |> 
    dplyr::ungroup() |> 
    tidyr::pivot_wider(names_from = sample_id, values_from = y.i, values_fill = NULL) |> 
    dplyr::select(-c(species))
  )

# Convert df into list of numeric vectors 
sp1_list <- lapply(as.list(sp1), na.omit) 

# Preview list of vectors 
lapply(sp1_list, function(x) head(x, 10))

# [SE2] ESTIMATE BIODIVERSITY
Sys.time()
# 5 mins for q=0
# 7 mins for q=c(0,1,2)

sp1_output <- 
  rbind(
    iNEXT::estimateD(x = sp1_list, q = c(0), datatype="incidence_freq", base="coverage", level=NULL, conf=0.95)
    ,
    iNEXT::estimateD(x = sp1_list, q = c(1,2), datatype="incidence_freq", base="coverage", level=0.9, conf=0.95)
  )
Sys.time()

# RESULT FOREST PLOT 
plot_sp1_output <- 
sp1_output |> 
    dplyr::mutate(
    Assemblage_Group = sub("_[0-9]+$", "", Assemblage),
    Assemblage_Period = factor(if_else(is.na(stringr::str_extract(Assemblage, "\\d+")), "548", stringr::str_extract(Assemblage, "\\d+")), 
                                  levels = c("1029","983","937","891","845","799","753","707","661","548"))
    ) |> 
  # Plot
  ggplot(aes(x = Assemblage_Period, y = qD, color = Assemblage_Group)) + 
  geom_point(position = position_dodge(width = 0.5), size = 2) + 
  ggplot2::geom_errorbar(aes(ymin = qD.LCL, ymax = qD.UCL),
                         position = position_dodge(width = 0.5), width = 0.5 #, linewidth = 1 
  ) +
  scale_color_manual(values = report_pal[c("burnt_after","unburnt_after","burnt_before","unburnt_before")],
                     name = "Sampling region and period:") +
  facet_wrap(~Order.q, scales = "free_y",
             strip.position = "top",
             labeller = as_labeller(c(`0` = "Species Richness", `1` = "Shannon Diversity", `2` = "Simpson Diversity")) ) + 
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "bottom", aspect.ratio = 1/1) +
  labs(title = "across XXX taxon groupings and XXX habitats",
       x = "Assemblage Period", y="")
plot_sp1_output
# Save as png
ggsave(filename = "plot_sp1_output.png",plot = plot_sp1_output, dpi = "print",path = here::here("image"),
       width = 2400, height = 1080, units = "px")
```

#### sp2: burnt_after & unburnt_after by tsf_period

```{r}
# [sp2] burnt_after & unburnt_after by tsf_period <start> -------------------------------------------
sp2 <-
  #[1] T (NUMBER OF SAMPLING UNITS)
  sighting_final |>
  filter(sample_id %in% c("burnt_after", "unburnt_after")) |>
  group_by(sample_id, tsf_period) |>
  summarise(T = n_distinct(gridcell_id), .groups = "drop") |>
  pivot_wider(
    names_from = c(sample_id, tsf_period),
    values_from = T,
    values_fill = NULL
  ) |>
  dplyr::mutate(species = "T") |>
  dplyr::select(species, everything()) |>
  rbind(
    #[2] SPECIES INCIDENCE FREQUENCIES: Number of grid cells in which ith species was sighted
    sighting_final |>
      filter(sample_id %in% c("burnt_after", "unburnt_after")) |>
      group_by(sample_id, tsf_period, species) |>
      summarise(y.i = n_distinct(gridcell_id), .groups = "drop") |>
      dplyr::ungroup() |>
      tidyr::pivot_wider(
        names_from = c(sample_id, tsf_period),
        values_from = y.i,
        values_fill = NULL
      )
  ) |>
  dplyr::select(-c(species))

# Convert df into list of numeric vectors 
sp2_list <- lapply(
  as.list(
    sp2), 
    na.omit) 
# Preview list of vectors 
lapply(sp2_list, function(x) head(x, 10))

# [SE2] ESTIMATE BIODIVERSITY

# [SE2.2] Estimate diversity with the suggested base coverage
runtime_start <- Sys.time() # Time difference of 5.27859 mins
sp2_output <- 
  rbind(
    iNEXT::estimateD(x = sp2_list, q = c(0), datatype="incidence_freq", base="coverage", level=NULL, conf=0.95)
    ,
    iNEXT::estimateD(x = sp2_list, q = c(1,2), datatype="incidence_freq", base="coverage", level=0.9, conf=0.95)
  )
runtime_end <- Sys.time()
runtime_end - runtime_start
rm(runtime_start,runtime_end) 

# [SE3] RESULTS PLOT 
sp2_output

sp2_output |>
  dplyr::mutate(
    sample_id = str_extract(Assemblage, "^[a-z]+_[a-z]+"), 
    tsf_period = factor(stringr::str_extract(Assemblage, "\\d+-\\d+"),
                        levels = c("0-3","3-6","6-9","9-12","12-15","15-18"))
    ) |>
  # Plot
  ggplot(aes(x = tsf_period, y = qD, color = sample_id)) + 
  geom_point(position = position_dodge(width = 0.5), size = 2) + 
  ggplot2::geom_errorbar(aes(ymin = qD.LCL, ymax = qD.UCL),
                         position = position_dodge(width = 0.5), width = 0.5 #, linewidth = 1 
  ) +
  scale_color_manual(values = report_pal[c("burnt_after","unburnt_after","burnt_before","unburnt_before")],
                     name = "Sampling region and period:") +
  facet_wrap(~Order.q, scales = "free_y",
             strip.position = "top",
             labeller = as_labeller(c(`0` = "Species Richness", `1` = "Shannon Diversity", `2` = "Simpson Diversity")) ) + 
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "bottom", aspect.ratio = 1/1) +
  labs(title = "Species diversity estimates for up to 18 months after the 2019/2020 wildfires", 
       x = "Months since fire", y = "")
# [sp2] burnt_after & unburnt_after by tsf_period <end> -------------------------------------------
```


<!-- Hsieh, T. C., Ma, K. H., & Chao, A. (2016). iNEXT: An R package for rarefaction and extrapolation of species diversity (Hill numbers). Methods in Ecology and Evolution, 7(12), 1451–1456. (https://doi.org/10.1111/2041-210X.12613) 
- Application 2: An a-symptotic analysis: comparison of the estimated asymptotic diversities. It is based on statistical estimation of Hill numbers via three functions: ChaoRichness(), ChaoShannon() and ChaoSimpson(); these functions return, respectively, the estimated asymptote for Hill numbers of order q = 0, 1 and 2 (see Kendrick et al. 2015, for ant data).
-->

#### sp3: Species Group  

```{r}
# [SE1] TRANSFORM INTO iNEXT datatype="incidence_freq" SUPPORTED INPUT FORMAT ---
sp3 <-
#[1] T (NUMBER OF SAMPLING UNITS = Number of grid cell)
sighting_final |>
  group_by(species_group, sample_id) |> 
  summarise(T = n_distinct(gridcell_id), .groups="drop") |> 
  pivot_wider(names_from = sample_id, values_from = T, values_fill = NULL) |> 
  filter(species_group != "Molluscs") |> # No sighting in burnt_before_xxxx 
  dplyr::mutate(species = "T") |> 
  select(species_group, species, everything()) |> 
  rbind(
    #[2] SPECIES INCIDENCE FREQUENCIES: Number of grid cells in which ith species was sighted
    sighting_final |> 
      filter(species_group != "Molluscs") |> # No sighting in burnt_before_xxxx
      group_by(sample_id, species_group, species) |> 
      summarise(y.i = n_distinct(gridcell_id), .groups="drop") |> 
      tidyr::pivot_wider(names_from = sample_id, values_from = y.i, values_fill = NULL) 
  ) |> 
  arrange(species_group)
```

##### sp3_amphibians  

```{r}
# [3.x] species_group=="Amphibians" <start> -----------------------------------------

# Convert df into list of numeric vectors 
sp3_amphibians_list <- lapply(
  as.list(
    sp3 |>
      filter(species_group == "Amphibians") |>
      select(-c(species_group, species))
  ), 
  na.omit) 

# Preview list of vectors 
lapply(sp3_amphibians_list, function(x) head(x, 10))

# [SE2] ESTIMATE BIODIVERSITY

# [SE2.1] Get Base coverage 
# sp3_amphibians_iNEXT <- iNEXT::iNEXT(sp3_amphibians_list, q=0, datatype="incidence_freq")
# 
# # Step 3. The suggested base coverage is the maximum of C_a and C_b
# sp3_amphibians_level <- max(
#   # Step 1. Compute the maximum coverage of reference sample sizes
#   max(sp3_amphibians_iNEXT$DataInfo$SC),
#   # Step 2. Compute the minimum coverage of r times reference sample sizes. r = 2
#   sp3_amphibians_iNEXT$iNextEst$coverage_based |>
#     dplyr::group_by(Assemblage) |>
#     dplyr::slice_max(t, n = 1) %>%
#     dplyr::ungroup() |>
#     select(SC) |>
#     min()
# )
# sp3_amphibians_level

# [SE2.2] Estimate diversity with the suggested base coverage
runtime_start <- Sys.time() # 5 mins for q=0 | 7 mins for q=c(0,1,2)
sp3_amphibians_output <- 
  rbind(
    iNEXT::estimateD(x = sp3_amphibians_list, q = c(0), datatype="incidence_freq", base="coverage", level=NULL, conf=0.95)
    ,
    iNEXT::estimateD(x = sp3_amphibians_list, q = c(1,2), datatype="incidence_freq", base="coverage", level=0.9, conf=0.95)
  )
runtime_end <- Sys.time()
runtime_end - runtime_start
rm(runtime_start,runtime_end)

# [SE3] RESULTS PLOT 
sp3_amphibians_output

sp3_amphibians_output |>
  dplyr::mutate(
    Assemblage_Group = sub("_[0-9]+$", "", Assemblage),
    Assemblage_Period = factor(if_else(is.na(stringr::str_extract(Assemblage, "\\d+")), "548", stringr::str_extract(Assemblage, "\\d+")), 
                               levels = c("1029","983","937","891","845","799","753","707","661","548"))
  ) |> 
  # Plot
  ggplot(aes(x = Assemblage_Period, y = qD, color = Assemblage_Group)) + 
  geom_point(position = position_dodge(width = 0.5), size = 2) + 
  ggplot2::geom_errorbar(aes(ymin = qD.LCL, ymax = qD.UCL),
                         position = position_dodge(width = 0.5), width = 0.5 #, linewidth = 1 
  ) +
  scale_color_manual(values = report_pal[c("burnt_after","unburnt_after","burnt_before","unburnt_before")],
                     name = "Sampling region and period:") +
  facet_wrap(~Order.q, scales = "free_y",
             strip.position = "top",
             labeller = as_labeller(c(`0` = "Species Richness", `1` = "Shannon Diversity", `2` = "Simpson Diversity")) ) + 
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "bottom", aspect.ratio = 1/1) + 
  labs(title = "Amphibian species diversity estimates", 
       x = "Assamblage Period", y = "Diversity (qD)")

# [3.x] species_group=="Amphibians" <end> -------------------------------------------
```


As such, we infer the strength of differences between treatments by categorizing  results by: “no overlap,” “partial overlap,” and “near-complete/complete overlap” of 95% confidence intervals (CI).

q=0  
No change in species richness between before-after fire and burnt-unburnt grid cells.  

Higher Shannon Diversity in burnt-after than un-burnt after -> indicates more diversity and evenness in the community structure. 
But this difference is not significant as 

q=2  
Higher Simpson Diversity (q=2) in burnt-after compared to unburnt-after regions. -> suggests that abundance is more evenly distributed among the common species, rather than being dominated by just one or two very abundant species

```{r}
sighting_final |>
  filter(species_group == "Amphibians") |> 
  group_by(sample_id, species) |>
  summarise(obs_per_species = n()) |> 
  filter(sample_id %in% c("burnt_after", "unburnt_after")) |> # arrange(desc(obs_per_species))
  # Plot 
  ggplot(aes(fill=species, y=obs_per_species, x=sample_id)) + 
    geom_bar(position="fill", stat="identity") +
  theme(legend.position = "bottom")
```


##### sp3_birds

```{r}
# [3.x] species_group=="Birds" <start> -----------------------------------------
# Convert df into list of numeric vectors 
sp3_birds_list <- lapply(
  as.list(
    sp3 |> 
      filter(species_group=="Birds") |> 
      select(-c(species_group, species))
    ), 
    na.omit) 

# Preview list of vectors 
lapply(sp3_birds_list, function(x) head(x, 10))

# [SE2] ESTIMATE BIODIVERSITY

# [SE2.1] Get Base coverage 
sp3_birds_iNEXT <- iNEXT::iNEXT(sp3_birds_list, q=0, datatype="incidence_freq")

# Step 3. The suggested base coverage is the maximum of C_a and C_b
sp3_birds_level <- max(
  # Step 1. Compute the maximum coverage of reference sample sizes
  max(sp3_birds_iNEXT$DataInfo$SC),
  # Step 2. Compute the minimum coverage of r times reference sample sizes. r = 2
  sp3_birds_iNEXT$iNextEst$coverage_based |>
    dplyr::group_by(Assemblage) |>
    dplyr::slice_max(t, n = 1) %>%
    dplyr::ungroup() |>
    select(SC) |>
    min()
)
sp3_birds_level

# [SE2.2] Estimate diversity with the suggested base coverage
runtime_start <- Sys.time() # 5 mins for q=0 | 7 mins for q=c(0,1,2)
sp3_birds_output <- 
  rbind(
    iNEXT::estimateD(x = sp3_birds_list, q = c(0), datatype="incidence_freq", base="coverage", level=NULL, conf=0.95)
    ,
    iNEXT::estimateD(x = sp3_birds_list, q = c(1,2), datatype="incidence_freq", base="coverage", level=0.9, conf=0.95)
  )
runtime_end <- Sys.time()
runtime_end - runtime_start
rm(runtime_start,runtime_end)

# [SE3] RESULTS PLOT 
sp3_birds_output

sp3_birds_output |>
  dplyr::mutate(
    Assemblage_Group = sub("_[0-9]+$", "", Assemblage),
    Assemblage_Period = factor(if_else(is.na(stringr::str_extract(Assemblage, "\\d+")), "548", stringr::str_extract(Assemblage, "\\d+")), 
                                  levels = c("1029","983","937","891","845","799","753","707","661","548"))
    ) |> 
  # Plot
  ggplot(aes(x = Assemblage_Period, y = qD, color = Assemblage_Group)) + 
  geom_point(position = position_dodge(width = 0.5), size = 2) + 
  ggplot2::geom_errorbar(aes(ymin = qD.LCL, ymax = qD.UCL),
                         position = position_dodge(width = 0.5), width = 0.5 #, linewidth = 1 
  ) +
  scale_color_manual(values = report_pal[c("burnt_after","unburnt_after","burnt_before","unburnt_before")],
                     name = "Sampling region and period:") +
  facet_wrap(~Order.q, scales = "free_y",
             strip.position = "top",
             labeller = as_labeller(c(`0` = "Species Richness", `1` = "Shannon Diversity", `2` = "Simpson Diversity")) ) + 
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "bottom", aspect.ratio = 1/1) + 
  labs(title = "Bird species diversity estimates", 
       x = "Assamblage Period", y = "Diversity (qD)")

# [3.x] species_group=="Birds" <end> -------------------------------------------
```

After the fire, there is significant decline in Bird species richness in burnt regions comparing to unburnt regions.

##### sp3_insects 

```{r}
# [3.x] species_group=="Insects" <start> -----------------------------------------
# Convert df into list of numeric vectors 
sp3_insects_list <- lapply(
  as.list(
    sp3 |> 
      filter(species_group=="Insects") |> 
      select(-c(species_group, species))
  ), 
  na.omit) 

# Preview list of vectors 
lapply(sp3_insects_list, function(x) head(x, 10))

# [SE2] ESTIMATE BIODIVERSITY

# [SE2.1] Get Base coverage 
sp3_insects_iNEXT <- iNEXT::iNEXT(sp3_insects_list, q=0, datatype="incidence_freq")

# Step 3. The suggested base coverage is the maximum of C_a and C_b
sp3_insects_level <- max(
  # Step 1. Compute the maximum coverage of reference sample sizes
  max(sp3_insects_iNEXT$DataInfo$SC),
  # Step 2. Compute the minimum coverage of r times reference sample sizes. r = 2
  sp3_insects_iNEXT$iNextEst$coverage_based |>
    dplyr::group_by(Assemblage) |>
    dplyr::slice_max(t, n = 1) %>%
    dplyr::ungroup() |>
    select(SC) |>
    min()
)
sp3_insects_level

# [SE2.2] Estimate diversity with the suggested base coverage
runtime_start <- Sys.time() # 5 mins for q=0 | 7 mins for q=c(0,1,2)
sp3_insects_output <- 
  rbind(
    iNEXT::estimateD(x = sp3_insects_list, q = c(0), datatype="incidence_freq", base="coverage", level=NULL, conf=0.95)
    ,
    iNEXT::estimateD(x = sp3_insects_list, q = c(1,2), datatype="incidence_freq", base="coverage", level=0.9, conf=0.95)
  )
runtime_end <- Sys.time()
runtime_end - runtime_start
rm(runtime_start,runtime_end)

# [SE3] RESULTS PLOT 
sp3_insects_output

sp3_insects_output |>
  dplyr::mutate(
    Assemblage_Group = sub("_[0-9]+$", "", Assemblage),
    Assemblage_Period = factor(if_else(is.na(stringr::str_extract(Assemblage, "\\d+")), "548", stringr::str_extract(Assemblage, "\\d+")), 
                               levels = c("1029","983","937","891","845","799","753","707","661","548"))
  ) |> 
  # Plot
  ggplot(aes(x = Assemblage_Period, y = qD, color = Assemblage_Group)) + 
  geom_point(position = position_dodge(width = 0.5), size = 2) + 
  ggplot2::geom_errorbar(aes(ymin = qD.LCL, ymax = qD.UCL),
                         position = position_dodge(width = 0.5), width = 0.5 #, linewidth = 1 
  ) +
  scale_color_manual(values = report_pal[c("burnt_after","unburnt_after","burnt_before","unburnt_before")],
                     name = "Sampling region and period:") +
  facet_wrap(~Order.q, scales = "free_y",
             strip.position = "top",
             labeller = as_labeller(c(`0` = "Species Richness", `1` = "Shannon Diversity", `2` = "Simpson Diversity")) ) + 
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "bottom", aspect.ratio = 1/1) + 
  labs(title = "Insects species diversity estimates", 
       x = "Assamblage Period", y = "Diversity (qD)")

# [3.x] species_group=="Insects" <end> -------------------------------------------
```



##### sp3_mammals  

```{r}
# [3.x] species_group=="Mammals" <start> -------------------------------------------

# Convert df into list of numeric vectors 
sp3_mammals_list <- lapply(
  as.list(
    sp3 |> 
      filter(species_group=="Mammals") |> 
      select(-c(species_group, species))
  ), 
  na.omit) 

# Preview list of vectors 
lapply(sp3_mammals_list, function(x) head(x, 10))

# [SE2] ESTIMATE BIODIVERSITY

# [SE2.1] Get Base coverage 
sp3_mammals_iNEXT <- iNEXT::iNEXT(sp3_mammals_list, q=0, datatype="incidence_freq")

# Step 3. The suggested base coverage is the maximum of C_a and C_b
sp3_mammals_level <- max(
  # Step 1. Compute the maximum coverage of reference sample sizes
  max(sp3_mammals_iNEXT$DataInfo$SC),
  # Step 2. Compute the minimum coverage of r times reference sample sizes. r = 2
  sp3_mammals_iNEXT$iNextEst$coverage_based |>
    dplyr::group_by(Assemblage) |>
    dplyr::slice_max(t, n = 1) %>%
    dplyr::ungroup() |>
    select(SC) |>
    min()
)
sp3_mammals_level

# [SE2.2] Estimate diversity with the suggested base coverage
runtime_start <- Sys.time() # 5 mins for q=0 | 7 mins for q=c(0,1,2)
sp3_mammals_output <- 
  rbind(
    iNEXT::estimateD(x = sp3_mammals_list, q = c(0), datatype="incidence_freq", base="coverage", level=NULL, conf=0.95)
    ,
    iNEXT::estimateD(x = sp3_mammals_list, q = c(1,2), datatype="incidence_freq", base="coverage", level=0.9, conf=0.95)
  )
runtime_end <- Sys.time()
runtime_end - runtime_start
rm(runtime_start,runtime_end)

# [SE3] RESULTS PLOT 
sp3_mammals_output

sp3_mammals_output |>
  dplyr::mutate(
    Assemblage_Group = sub("_[0-9]+$", "", Assemblage),
    Assemblage_Period = factor(if_else(is.na(stringr::str_extract(Assemblage, "\\d+")), "548", stringr::str_extract(Assemblage, "\\d+")), 
                               levels = c("1029","983","937","891","845","799","753","707","661","548"))
  ) |> 
  # Plot
  ggplot(aes(x = Assemblage_Period, y = qD, color = Assemblage_Group)) + 
  geom_point(position = position_dodge(width = 0.5), size = 2) + 
  ggplot2::geom_errorbar(aes(ymin = qD.LCL, ymax = qD.UCL),
                         position = position_dodge(width = 0.5), width = 0.5 #, linewidth = 1 
  ) +
  scale_color_manual(values = report_pal[c("burnt_after","unburnt_after","burnt_before","unburnt_before")],
                     name = "Sampling region and period:") +
  facet_wrap(~Order.q, scales = "free_y",
             strip.position = "top",
             labeller = as_labeller(c(`0` = "Species Richness", `1` = "Shannon Diversity", `2` = "Simpson Diversity")) ) + 
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "bottom", aspect.ratio = 1/1) +
    labs(title = "Mammal species diversity estimates", 
       x = "Assamblage Period", y = "")

# [3.x] species_group=="Mammals" <end> -------------------------------------------
```
##### sp3_plants  

```{r}
# [3.x] species_group=="Plants" <start> -----------------------------------------
# Convert df into list of numeric vectors 
sp3_plants_list <- lapply(
  as.list(
    sp3 |> 
      filter(species_group=="Plants") |> 
      select(-c(species_group, species))
  ), 
  na.omit) 

# Preview list of vectors 
# lapply(sp3_plants_list, function(x) head(x, 10))

# [SE2] ESTIMATE BIODIVERSITY

# [SE2.1] Get Base coverage 
# sp3_plants_iNEXT <- iNEXT::iNEXT(sp3_plants_list, q=0, datatype="incidence_freq")
# # Step 3. The suggested base coverage is the maximum of C_a and C_b
# sp3_plants_level <- max(
#   # Step 1. Compute the maximum coverage of reference sample sizes
#   max(sp3_plants_iNEXT$DataInfo$SC),
#   # Step 2. Compute the minimum coverage of r times reference sample sizes. r = 2
#   sp3_plants_iNEXT$iNextEst$coverage_based |>
#     dplyr::group_by(Assemblage) |>
#     dplyr::slice_max(t, n = 1) %>%
#     dplyr::ungroup() |>
#     select(SC) |>
#     min()
# )
# sp3_plants_level

# [SE2.2] Estimate diversity with the suggested base coverage
runtime_start <- Sys.time() # 5 mins for q=0 | 7 mins for q=c(0,1,2)
sp3_plants_output <- 
  rbind(
    iNEXT::estimateD(x = sp3_plants_list, q = c(0), datatype="incidence_freq", base="coverage", level=NULL, conf=0.95)
    ,
    iNEXT::estimateD(x = sp3_plants_list, q = c(1,2), datatype="incidence_freq", base="coverage", level=0.9, conf=0.95)
  )
runtime_end <- Sys.time()
runtime_end - runtime_start
rm(runtime_start,runtime_end)

# [SE3] RESULTS PLOT 
sp3_plants_output

sp3_plants_output |>
  dplyr::mutate(
    Assemblage_Group = sub("_[0-9]+$", "", Assemblage),
    Assemblage_Period = factor(if_else(is.na(stringr::str_extract(Assemblage, "\\d+")), "548", stringr::str_extract(Assemblage, "\\d+")), 
                               levels = c("1029","983","937","891","845","799","753","707","661","548"))
  ) |> 
  # Plot
  ggplot(aes(x = Assemblage_Period, y = qD, color = Assemblage_Group)) + 
  geom_point(position = position_dodge(width = 0.5), size = 2) + 
  ggplot2::geom_errorbar(aes(ymin = qD.LCL, ymax = qD.UCL),
                         position = position_dodge(width = 0.5), width = 0.5 #, linewidth = 1 
  ) +
  scale_color_manual(values = report_pal[c("burnt_after","unburnt_after","burnt_before","unburnt_before")],
                     name = "Sampling region and period:") +
  facet_wrap(~Order.q, scales = "free_y",
             strip.position = "top",
             labeller = as_labeller(c(`0` = "Species Richness", `1` = "Shannon Diversity", `2` = "Simpson Diversity")) ) + 
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "bottom", aspect.ratio = 1/1) + 
  labs(title = "Plants species diversity estimates", 
       x = "Assamblage Period", y = "Diversity (qD)")

# [3.x] species_group=="Plants" <end> -------------------------------------------
```

##### sp3_reptiles  

```{r}
# [3.x] species_group=="Reptiles" <start> -----------------------------------------
# Convert df into list of numeric vectors 
sp3_reptiles_list <- lapply(
  as.list(
    sp3 |> 
      filter(species_group=="Reptiles") |> 
      select(-c(species_group, species))
  ), 
  na.omit) 

# Preview list of vectors 
# lapply(sp3_reptiles_list, function(x) head(x, 10))

# [SE2] ESTIMATE BIODIVERSITY

# [SE2.1] Get Base coverage 
# sp3_reptiles_iNEXT <- iNEXT::iNEXT(sp3_reptiles_list, q=0, datatype="incidence_freq")
# # Step 3. The suggested base coverage is the maximum of C_a and C_b
# sp3_reptiles_level <- max(
#   # Step 1. Compute the maximum coverage of reference sample sizes
#   max(sp3_reptiles_iNEXT$DataInfo$SC),
#   # Step 2. Compute the minimum coverage of r times reference sample sizes. r = 2
#   sp3_reptiles_iNEXT$iNextEst$coverage_based |>
#     dplyr::group_by(Assemblage) |>
#     dplyr::slice_max(t, n = 1) %>%
#     dplyr::ungroup() |>
#     select(SC) |>
#     min()
# )
# sp3_reptiles_level

# [SE2.2] Estimate diversity with the suggested base coverage
runtime_start <- Sys.time() # 5 mins for q=0 | 7 mins for q=c(0,1,2)
sp3_reptiles_output <- 
  rbind(
    iNEXT::estimateD(x = sp3_reptiles_list, q = c(0), datatype="incidence_freq", base="coverage", level=NULL, conf=0.95)
    ,
    iNEXT::estimateD(x = sp3_reptiles_list, q = c(1,2), datatype="incidence_freq", base="coverage", level=0.9, conf=0.95)
  )
runtime_end <- Sys.time()
runtime_end - runtime_start
rm(runtime_start,runtime_end)

# [SE3] RESULTS PLOT 
sp3_reptiles_output

sp3_reptiles_output |>
  dplyr::mutate(
    Assemblage_Group = sub("_[0-9]+$", "", Assemblage),
    Assemblage_Period = factor(if_else(is.na(stringr::str_extract(Assemblage, "\\d+")), "548", stringr::str_extract(Assemblage, "\\d+")), 
                               levels = c("1029","983","937","891","845","799","753","707","661","548"))
  ) |> 
  # Plot
  ggplot(aes(x = Assemblage_Period, y = qD, color = Assemblage_Group)) + 
  geom_point(position = position_dodge(width = 0.5), size = 2) + 
  ggplot2::geom_errorbar(aes(ymin = qD.LCL, ymax = qD.UCL),
                         position = position_dodge(width = 0.5), width = 0.5 #, linewidth = 1 
  ) +
  scale_color_manual(values = report_pal[c("burnt_after","unburnt_after","burnt_before","unburnt_before")],
                     name = "Sampling region and period:") +
  facet_wrap(~Order.q, scales = "free_y",
             strip.position = "top",
             labeller = as_labeller(c(`0` = "Species Richness", `1` = "Shannon Diversity", `2` = "Simpson Diversity")) ) + 
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "bottom", aspect.ratio = 1/1) + 
  labs(title = "Reptiles species diversity estimates", 
       x = "Assamblage Period", y = "Diversity (qD)")

# [3.x] species_group=="Reptiles" <end> -------------------------------------------
```


#### sp4: Vegetation Group  

```{r}
# [SE1] TRANSFORM INTO iNEXT datatype="incidence_freq" SUPPORTED INPUT FORMAT ---
sp4 <-
#[1] T (NUMBER OF SAMPLING UNITS = Number of grid cell)
sighting_final |>
  # Filter-out
  dplyr::filter(!evc_group %in% c("Riverine Grassy Woodlands or Forests","Rocky Outcrop or Escarpment Scrubs")) |> 
  group_by(evc_group, sample_id) |> 
  summarise(T = n_distinct(gridcell_id), .groups="drop") |> 
  pivot_wider(names_from = sample_id, values_from = T, values_fill = NULL) |> 
  dplyr::mutate(species = "T") |> 
  select(evc_group, species, everything()) |>
  rbind(
    #[2] SPECIES INCIDENCE FREQUENCIES: Number of grid cells in which ith species was sighted
    sighting_final |>
      dplyr::filter(!evc_group %in% c("Riverine Grassy Woodlands or Forests","Rocky Outcrop or Escarpment Scrubs")) |> 
      group_by(sample_id, evc_group, species) |> 
      summarise(y.i = n_distinct(gridcell_id), .groups="drop") |> 
      tidyr::pivot_wider(names_from = sample_id, values_from = y.i, values_fill = NULL) 
  ) |> 
  arrange(evc_group)
```
##### sp4_DryForests

```{r}
# [4.x] evc_group=="Dry Forests" <start> -----------------------------------------
# Convert df into list of numeric vectors 
sp4_DryForests_list <- lapply(
  as.list(
    sp4 |> 
      filter(evc_group=="Dry Forests") |> 
      select(-c(evc_group, species))
  ), 
  na.omit) 

# Preview list of vectors 
# lapply(sp4_DryForests_list, function(x) head(x, 10))

# [SE2] ESTIMATE BIODIVERSITY

# [SE2.1] Get Base coverage 
# sp4_DryForests_iNEXT <- iNEXT::iNEXT(sp4_DryForests_list, q=0, datatype="incidence_freq")
# # Step 3. The suggested base coverage is the maximum of C_a and C_b
# sp4_DryForests_level <- max(
#   # Step 1. Compute the maximum coverage of reference sample sizes
#   max(sp4_DryForests_iNEXT$DataInfo$SC),
#   # Step 2. Compute the minimum coverage of r times reference sample sizes. r = 2
#   sp4_DryForests_iNEXT$iNextEst$coverage_based |>
#     dplyr::group_by(Assemblage) |>
#     dplyr::slice_max(t, n = 1) %>%
#     dplyr::ungroup() |>
#     select(SC) |>
#     min()
# )
# sp4_DryForests_level

# [SE2.2] Estimate diversity with the suggested base coverage
runtime_start <- Sys.time() # 5 mins for q=0 | 7 mins for q=c(0,1,2)
sp4_DryForests_output <- 
  rbind(
    iNEXT::estimateD(x = sp4_DryForests_list, q = c(0), datatype="incidence_freq", base="coverage", level=NULL, conf=0.95)
    ,
    iNEXT::estimateD(x = sp4_DryForests_list, q = c(1,2), datatype="incidence_freq", base="coverage", level=0.9, conf=0.95)
  )
runtime_end <- Sys.time()
runtime_end - runtime_start
rm(runtime_start,runtime_end)

# [SE3] RESULTS PLOT 
sp4_DryForests_output

sp4_DryForests_output |>
  dplyr::mutate(
    Assemblage_Group = sub("_[0-9]+$", "", Assemblage),
    Assemblage_Period = factor(if_else(is.na(stringr::str_extract(Assemblage, "\\d+")), "548", stringr::str_extract(Assemblage, "\\d+")), 
                               levels = c("1029","983","937","891","845","799","753","707","661","548"))
  ) |> 
  # Plot
  ggplot(aes(x = Assemblage_Period, y = qD, color = Assemblage_Group)) + 
  geom_point(position = position_dodge(width = 0.5), size = 2) + 
  ggplot2::geom_errorbar(aes(ymin = qD.LCL, ymax = qD.UCL),
                         position = position_dodge(width = 0.5), width = 0.5 #, linewidth = 1 
  ) +
  scale_color_manual(values = report_pal[c("burnt_after","unburnt_after","burnt_before","unburnt_before")],
                     name = "Sampling region and period:") +
  facet_wrap(~Order.q, scales = "free_y",
             strip.position = "top",
             labeller = as_labeller(c(`0` = "Species Richness", `1` = "Shannon Diversity", `2` = "Simpson Diversity")) ) + 
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "bottom", aspect.ratio = 1/1) + 
  labs(title = "Dry Forests - diversity estimates", 
       x = "Assamblage Period", y = "")

# [4.x] evc_group=="Dry Forests" <end> -------------------------------------------
```

##### sp4_Heathlands 
```{r}
# [4.x] evc_group=="Heathlands" <start> -----------------------------------------
# Convert df into list of numeric vectors 
sp4_Heathlands_list <- lapply(
  as.list(
    sp4 |> 
      filter(evc_group=="Heathlands") |> 
      select(-c(evc_group, species))
  ), 
  na.omit) 

# Preview list of vectors 
# lapply(sp4_Heathlands_list, function(x) head(x, 10))

# [SE2] ESTIMATE BIODIVERSITY

# [SE2.1] Get Base coverage 
# sp4_Heathlands_iNEXT <- iNEXT::iNEXT(sp4_Heathlands_list, q=0, datatype="incidence_freq")
# # Step 3. The suggested base coverage is the maximum of C_a and C_b
# sp4_Heathlands_level <- max(
#   # Step 1. Compute the maximum coverage of reference sample sizes
#   max(sp4_Heathlands_iNEXT$DataInfo$SC),
#   # Step 2. Compute the minimum coverage of r times reference sample sizes. r = 2
#   sp4_Heathlands_iNEXT$iNextEst$coverage_based |>
#     dplyr::group_by(Assemblage) |>
#     dplyr::slice_max(t, n = 1) %>%
#     dplyr::ungroup() |>
#     select(SC) |>
#     min()
# )
# sp4_Heathlands_level

# [SE2.2] Estimate diversity with the suggested base coverage
runtime_start <- Sys.time() # 5 mins for q=0 | 7 mins for q=c(0,1,2)
sp4_Heathlands_output <- 
  rbind(
    iNEXT::estimateD(x = sp4_Heathlands_list, q = c(0), datatype="incidence_freq", base="coverage", level=NULL, conf=0.95)
    ,
    iNEXT::estimateD(x = sp4_Heathlands_list, q = c(1,2), datatype="incidence_freq", base="coverage", level=0.9, conf=0.95)
  )
runtime_end <- Sys.time()
runtime_end - runtime_start
rm(runtime_start,runtime_end)

# [SE3] RESULTS PLOT 
sp4_Heathlands_output

sp4_Heathlands_output |>
  dplyr::mutate(
    Assemblage_Group = sub("_[0-9]+$", "", Assemblage),
    Assemblage_Period = factor(if_else(is.na(stringr::str_extract(Assemblage, "\\d+")), "548", stringr::str_extract(Assemblage, "\\d+")), 
                               levels = c("1029","983","937","891","845","799","753","707","661","548"))
  ) |> 
  # Plot
  ggplot(aes(x = Assemblage_Period, y = qD, color = Assemblage_Group)) + 
  geom_point(position = position_dodge(width = 0.5), size = 2) + 
  ggplot2::geom_errorbar(aes(ymin = qD.LCL, ymax = qD.UCL),
                         position = position_dodge(width = 0.5), width = 0.5 #, linewidth = 1 
  ) +
  scale_color_manual(values = report_pal[c("burnt_after","unburnt_after","burnt_before","unburnt_before")],
                     name = "Sampling region and period:") +
  facet_wrap(~Order.q, scales = "free_y",
             strip.position = "top",
             labeller = as_labeller(c(`0` = "Species Richness", `1` = "Shannon Diversity", `2` = "Simpson Diversity")) ) + 
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "bottom", aspect.ratio = 1/1) + 
  labs(title = "Heathlands - diversity estimates", 
       x = "Assamblage Period", y = "")

# [4.x] evc_group=="Heathlands" <end> -------------------------------------------
```



<!-- ----------------------------------------------------------------------- -->
<!-- ----------------------------------------------------------------------- -->
<!-- ----------------------------------------------------------------------- -->

# Appendix  
## Appendix 2  

<!--TODO-->
Reasoning behind choosing 548 days (18 months) as the post-fire period
(1) Ecological significance. 
(2) Data availability.
(3) Comparison to prior studies. 
(4) Practical considerations.