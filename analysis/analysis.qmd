...
```{r}
# Set seed for stratified sampling procedure
set.seed(5543)
...
burnt_after_gridcell_stratified_sampled <- 
  # Data source
  burnt_after_gridcell |> dplyr::filter(pfp_id==1) |> dplyr::select(evc_group, gridcell_id) |> 
  # Left join with sample_size_by_evc
  dplyr::left_join(sample_size_by_evc |> select(c(evc_group, sample_size, which_treatment)),
            by = "evc_group") |> 
...
# Checking
# burnt_after_gridcell_stratified_sampled |> group_by(evc_group) |> summarise(sample_size = n_distinct(gridcell_id))
...
rbind(
  ...
  dplyr::left_join(
    # [S2]
    ## x (left data)
    sighting |> sf::st_drop_geometry() |>  dplyr::filter(before_after_fire == "before"),
    ## y (right data)
    burnt_after_gridcell |> 
