...
rbind(
  ...
  sighting |> 
    sf::st_drop_geometry() |> 
    dplyr::filter(before_after_fire == "after" & time_since_fire_days <= 548,
                  gridcell_id %in%  unique(burnt_after_gridcell$gridcell_id)) |> 
    # Column structure
  ...
  dplyr::left_join(
    # [S2]
    sighting |> sf::st_drop_geometry() |>  dplyr::filter(before_after_fire == "before"), # x
    burnt_after_gridcell |> select(-c(fire_start_date, evc_group)), # y
...
# sighting_final |> group_by(evc_group) |> summarise(n_distinct_gc = n_distinct(gridcell_id)) 
# ------------------------------------------------------------------------------
...
dplyr::left_join(
  ...
  #[S5.1] Determine limiting factor for each group
  dplyr::mutate(sample_size = pmin(burnt_after_gridcell_count,  unburnt_after_gridcell_count)) 

sample_size_by_evc


# How many sighting per UNBURNT grid cell AFTER the fire? 
# sighting |> 
#   sf::st_drop_geometry() |> 
#   dplyr::filter(burnt == "burnt" & before_after_fire == "after") |> 
