...
sp1_output <- 
  rbind(
    ...
    # ,
    # iNEXT::estimateD(x = sp1_list, q = c(1,2), datatype="incidence_freq", base="coverage", level=1, conf=0.95),
  )
Sys.time()

sp1_output_q12 <- iNEXT::estimateD(x = sp1_list, q = c(1,2), datatype="incidence_freq", base="coverage", level=0.9, conf=0.95)
Sys.time()

rbind(sp1_output, sp1_output_q12)
# FOREST PLOT 
...
sighting_final |>
  ...
  rbind(
    ...
    sighting_final |> 
      ...
      summarise(y.i = n_distinct(gridcell_id), .groups="drop") |> 
      # filter(species == "Acacia dealbata") 
      tidyr::pivot_wider(names_from = sample_id, values_from = y.i, values_fill = NULL) 
  ) |> 
...
```{r}
# [1.x] species_group=="Amphibians" <start> -----------------------------------------
...
# [1.x] species_group=="Amphibians" <end> -------------------------------------------
```
...
```{r}
# [1.x] species_group=="Insects" <start> -----------------------------------------
# Convert df into list of numeric vectors 
...
```{r}
# [1.x] species_group=="Mammals" <start> -------------------------------------------
...
# [1.x] species_group=="Mammals" <end> -------------------------------------------
```
##### sp3_plants  

```{r}
# [1.x] species_group=="Plants" <start> -----------------------------------------
# Convert df into list of numeric vectors 
...
# [1.x] species_group=="Plants" <end> -------------------------------------------
```

##### sp3_reptiles  

```{r}
# [1.x] species_group=="Reptiles" <start> -----------------------------------------
# Convert df into list of numeric vectors 
...
# [1.x] species_group=="Reptiles" <end> -------------------------------------------
```
