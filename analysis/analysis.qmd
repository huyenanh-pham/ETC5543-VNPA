...
burnt_after_gridcell <- sighting |>
  ...
  select(gridcell_id, fire_start_date) |>
  unique() |> # 11,168 unique gridcell_id
  ...
  ungroup() |> 
  dplyr::mutate(
    pfp_start_date = fire_start_date - 1029 + (pfp_id - 1) * 46,
    pfp_end_date   = pfp_start_date + 548) |> 
  # pull() 
  dplyr::filter(pfp_start_date <= as.Date(fire1920_timeframe$start_date[1]) - 548) # "2018-05-22" The latest pre-fire period began on 22 May 2018, ensuring no overlap with the post-fire periods.
  
sighting |> 
  sf::st_drop_geometry() |> 
    # [S2] Extract ALA sighting observations only from grid cells with post-fire observations 
  dplyr::filter(
    before_after_fire == "before",
    gridcell_id %in% burnt_after_gridcell[burnt_after_gridcell[["pfp_id"]] == 1, ][["gridcell_id"]]) |> 
  # group_by(gridcell_id) |> summarise(n=n())
  dplyr::filter(gridcell_id == "27203575") 
...
sighting |> 
  sf::st_drop_geometry() |> 
  # Column structure
  select(c("sighting_id", "species_group", "species", "sighting_date", "fire_season", 
           "fire_no", "fire_start_date", "fire_svrty", "evc_group", "before_after_fire", 
           "burnt", "sample", "time_since_fire_days", "gridcell_id")) |> 
  dplyr::filter(before_after_fire == "after" & time_since_fire_days <= 548) |> 
  dplyr::mutate(sample_id = "burnt_after")
  ,
  # part 2/4: sample_id = "burnt_before_xxxx"
  dplyr::left_join(sighting |> sf::st_drop_geometry() |>  dplyr::filter(before_after_fire == "before"), # x
                 burnt_after_gridcell |> select(-c(fire_start_date)), # y
  ...
  select(c("sighting_id", "species_group", "species", "sighting_date", "fire_season", 
           "fire_no", "fire_start_date", "fire_svrty", "evc_group", "before_after_fire", 
           "burnt", "sample", "time_since_fire_days", "gridcell_id","sample_id")) 
)
