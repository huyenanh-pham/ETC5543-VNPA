...
sp3_birds <- rbind(
  #[1] T (NUMBER OF SAMPLING UNITS)
  sighting_final |>
    dplyr::filter(species_group == "Birds") |>
    group_by(sample_id) |> 
    summarise(T = n_distinct(gridcell_id)) |> 
    pivot_wider(names_from = sample_id, values_from = T, values_fill = NULL)
  ,
...
# Convert df into list of numeric vectors 
sp3_birds_list <- lapply(as.list(sp3_birds), na.omit) 
...
```
```{r}
sp3_birds_output # SC_{q=0} 0.9854657
sp3_birds[1,] * 2
iNEXT::DataInfo(sp3_birds_list)$n
max(iNEXT::DataInfo(sp3_birds_list)$SC)
```
